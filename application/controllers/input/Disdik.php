...
class Disdik extends Admin_Controller {
	
	public function __construct() {
		parent::__construct();
		$this->load->model(['m_disdik','m_curd']);
		
	}
	public function index()
	{
		$this->vars['title']="Display Pengumuman Informasi Akademik";
		$this->vars['display']=$this->vars['disdik']=TRUE;
		$this->vars['data']=$this->m_disdik->get_agenda();
		$this->vars['content']='input/disdik';
		$this->load->view('backend/index',$this->vars);
	}
	
	public function load_new(){
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			$this->vars['modal_title']='Agenda Dinas Pendidikan dan Kebudayaan';
			return $this->load->view('input/modal_disdik',$this->vars);
		}else{
			redirect('input/disdik');
		}
	}
	...
	public function edit(){
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			$id=$_POST['id'];
			$this->vars['data_disdik']=$this->m_disdik->get_single_agenda($id);
			$this->vars['modal_title']='Agenda Dinas Pendidikan dan Kebudayaan';
			return $this->load->view('input/modal_disdik',$this->vars);
		}else{
			redirect('input/disdik');
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			if($this->validation()){		
				if(isset($_POST['id'])){
					...
					$hasil=$this->m_curd->update(array('id' => $id),'agenda_disdik',$filldata);
				}else{
					//new 
					$filldata=$this->security->xss_clean(array(
						'nama_agenda' => $_POST['nama_agenda'],
					...
					$hasil=$this->m_curd->add_new('agenda_disdik',$filldata);
				}
				if($hasil){
	...
	public function hapus($id){
		...
		if($id_agenda && $id_agenda !=0 && ctype_digit((string) $id_agenda)){
			$hasil = $this->m_curd->hapus(array('id' => $id_agenda),'agenda_disdik');
			if($hasil){
				$this->vars['type']='alert-success';
